...
| 后续授权迁移 | 先备份 `data/governance_workbench.sqlite` 并记录 hash，再执行 apply。rollback 脚本内置目标表空值 guard，并且仅允许通过 fail-fast SQL runner 用于 disposable copy，或确认本迁移新增表全部为空；任一表有数据时必须停止，除非另有明确 destructive authorization 与可核验 backup record。`npm run smoke:migration-gate` 在临时副本验证 populated rollback 被拒绝，以及 empty rollback/reapply 可执行。 |
